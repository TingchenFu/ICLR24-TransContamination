...
def parse_args():
    ...
    parser.add_argument("--max_train_steps_per_epoch", type=int, default=-1)
    ...
    parser.add_argument("--zh_proportion",type=int,default=1)
    parser.add_argument("--en_proportion",type=int,default=1)
...
def main():
    ...
    if not args.streaming:
        column_names = raw_zh_dataset.column_names
        text_column_name = "text" if "text" in column_names else column_names[0]
    else:
        column_names = ['timestamp','url','text',]
        #column_names = ['doc_no','file_no','text']
    ...
    # print(list(lm_en_dataset.take(2)))
    # print(list(lm_zh_dataset.take(2)))
    #print(lm_en_dataset.take(128).column_names)
                
    
    from datasets import interleave_datasets
    train_dataset = interleave_datasets([lm_en_dataset,lm_zh_dataset],probabilities=[args.en_proportion/(args.en_proportion + args.zh_proportion),args.zh_proportion/(args.en_proportion + args.zh_proportion)],seed = args.seed)
    ...
    if type(args.resume_from_checkpoint)== bool and args.resume_from_checkpoint is True:
        for file in os.listdir(args.output_dir):
            if 'step_ckpt' in file:
                args.resume_from_checkpoint = os.path.join(args.output_dir,file)
                resume_step = int(args.resume_from_checkpoint.split('/')[-1].replace('step_ckpt',''))
                #starting_epoch = resume_step // num_update_steps_per_epoch
    if type(args.resume_from_checkpoint) == str and os.path.exists(args.resume_from_checkpoint):
        logger.info(f"Resumed from checkpoint: {args.resume_from_checkpoint}")
        accelerator.load_state(args.resume_from_checkpoint)
        resume_step = int(args.resume_from_checkpoint.split('/')[-1].replace('step_ckpt',''))
        logger.info(f"Resumedstep: {resume_step}")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (not args.streaming and step == len(train_dataloader) - 1):
                ...
                if not args.streaming:
                    progress_bar.update(1)
                completed_steps += 1
                if args.max_train_steps_per_epoch!=-1 and completed_steps % args.max_train_steps_per_epoch ==0:
                    break
